...
bool acceptSequence(const TelemetryGroupStamp &stamp, uint32_t session, uint32_t seq, uint32_t now) {
  if (stamp.lastRxMs == 0U) return true;
  // A fresh v3 session is authoritative over legacy. Legacy may recover only
  // after the v3 source has been absent for >5 s. A new non-zero session resets
  // ordering immediately, allowing ROS/bridge restarts without false OOO drops.
  if (session != 0U && stamp.session != session) return true;
  if (session == 0U && stamp.session != 0U && static_cast<uint32_t>(now - stamp.lastRxMs) <= 5000U) return false;
  if (static_cast<uint32_t>(now - stamp.lastRxMs) > 5000U) return true;
  return static_cast<int32_t>(seq - stamp.seq) > 0;
}

void commitStamp(TelemetryGroupStamp &stamp, uint32_t session, uint32_t seq, uint32_t age,
...
bool parseEsc(const Header &h, char *f[], size_t n, VehicleTelemetry &t,
  ...
  if (std::strcmp(h.group, "PWR") == 0) {
    ...
    if (!acceptSequence(t.escx.power, h.session, h.seq, now)) { outOfOrder = true; return false; }
    commitStamp(t.escx.power, h.session, h.seq, h.age, valid, now);
    if (valid) {
  ...
  if (std::strcmp(h.group, "ENC") == 0) {
    ...
    if (!acceptSequence(t.escx.encoder, h.session, h.seq, now)) { outOfOrder = true; return false; }
    commitStamp(t.escx.encoder, h.session, h.seq, h.age, valid, now);
    if (valid) {
  ...
  if (std::strcmp(h.group, "LINK") == 0) {
    ...
    if (!acceptSequence(t.escx.link, h.session, h.seq, now)) { outOfOrder = true; return false; }
    commitStamp(t.escx.link, h.session, h.seq, h.age, valid, now);
  ...
  if (std::strcmp(h.group, "PERF") == 0) {
    ...
    if (!acceptSequence(t.escx.performance, h.session, h.seq, now)) { outOfOrder = true; return false; }
    commitStamp(t.escx.performance, h.session, h.seq, h.age, valid, now);
...
bool parsePer(const Header &h, char *f[], size_t n, VehicleTelemetry &t,
  ...
  if (std::strcmp(h.group, "CAM") == 0) {
    ...
    if (!acceptSequence(t.perx.camera,h.session,h.seq,now)) { outOfOrder=true; return false; }
    commitStamp(t.perx.camera,h.session,h.seq,h.age,valid,now);
  ...
  if (std::strcmp(h.group, "DET") == 0) {
    ...
    if (!acceptSequence(t.perx.detection,h.session,h.seq,now)) { outOfOrder=true; return false; }
    commitStamp(t.perx.detection,h.session,h.seq,h.age,valid,now);
  ...
  if (std::strcmp(h.group, "LANE") == 0) {
    ...
    if(!acceptSequence(t.perx.lane,h.session,h.seq,now)){outOfOrder=true;return false;}
    commitStamp(t.perx.lane,h.session,h.seq,h.age,valid,now);
  ...
  if (std::strcmp(h.group, "DRV") == 0) {
    ...
    if(!acceptSequence(t.perx.drivable,h.session,h.seq,now)){outOfOrder=true;return false;}
    commitStamp(t.perx.drivable,h.session,h.seq,h.age,valid,now);
  ...
  if (std::strcmp(h.group, "OBS") == 0) {
    ...
    if(!acceptSequence(t.perx.obstacle,h.session,h.seq,now)){outOfOrder=true;return false;}
    commitStamp(t.perx.obstacle,h.session,h.seq,h.age,valid,now);
  ...
  if (std::strcmp(h.group, "PERF") == 0) {
    ...
    if(!acceptSequence(t.perx.performance,h.session,h.seq,now)){outOfOrder=true;return false;}
    commitStamp(t.perx.performance,h.session,h.seq,h.age,valid,now);
...
bool parseNav(const Header &h, char *f[], size_t n, VehicleTelemetry &t,
  ...
  if (std::strcmp(h.group, "POSE") == 0) {
    ...
    if(!acceptSequence(t.navx.pose,h.session,h.seq,now)){outOfOrder=true;return false;} commitStamp(t.navx.pose,h.session,h.seq,h.age,valid,now);
  ...
  if (std::strcmp(h.group, "ODOM") == 0) {
    ...
    if(!acceptSequence(t.navx.odom,h.session,h.seq,now)){outOfOrder=true;return false;} commitStamp(t.navx.odom,h.session,h.seq,h.age,valid,now);
  ...
  if (std::strcmp(h.group, "IMU") == 0) {
    ...
    if(!acceptSequence(t.navx.imuMag,h.session,h.seq,now)){outOfOrder=true;return false;} commitStamp(t.navx.imuMag,h.session,h.seq,h.age,valid,now);
  ...
  if (std::strcmp(h.group, "NAV2") == 0) {
    ...
    if(!acceptSequence(t.navx.nav2,h.session,h.seq,now)){outOfOrder=true;return false;} commitStamp(t.navx.nav2,h.session,h.seq,h.age,valid,now);
  ...
  if (std::strcmp(h.group, "COST") == 0) {
    ...
    if(!acceptSequence(t.navx.costmap,h.session,h.seq,now)){outOfOrder=true;return false;} commitStamp(t.navx.costmap,h.session,h.seq,h.age,valid,now);
  ...
  if (std::strcmp(h.group, "CTRL") == 0) {
    ...
    if(!acceptSequence(t.navx.control,h.session,h.seq,now)){outOfOrder=true;return false;} commitStamp(t.navx.control,h.session,h.seq,h.age,valid,now);
...
ExtendedTelemetryParseResult parseExtendedTelemetryLine(
  ...
  const bool is_legacy = std::strncmp(line,"ESCX:",5)==0 || std::strncmp(line,"PERX:",5)==0 ||
                         std::strncmp(line,"NAVX:",5)==0;
  if (!is_v3 && !is_legacy) return result;
  result.recognized = true;
  result.v3 = is_v3;
  const size_t len = std::strlen(line);
  if (len == 0U || len > kMaxExtendedLine) { result.lengthError = true; return result; }
  ...
  if (is_v3) {
    const char *last_colon_src = std::strrchr(line, ':');
    if (last_colon_src == nullptr || last_colon_src <= line + 5) { result.lengthError = true; return result; }
    uint32_t expected_crc = 0U;
    if (!parseHex32(last_colon_src + 1, expected_crc)) { result.crcError = true; return result; }
    const size_t signed_len = static_cast<size_t>(last_colon_src - line);
    if (crc32Bytes(line, signed_len) != expected_crc) { result.crcError = true; return result; }
    buf[signed_len] = '\0';

    // F4X3:<domain>:<group>:3:<session>:<seq>:<age>:<payload_len>:<payload>
    char *parts[9]{}; size_t count=0U; char *cur=buf;
    while (count < 8U) {
      parts[count++] = cur;
      char *c=std::strchr(cur, ':');
      if (!c) { result.lengthError=true; return result; }
      *c='\0'; cur=c+1;
    }
    parts[count++] = cur;
    if (count != 9U || std::strcmp(parts[0],"F4X3")!=0) { result.lengthError=true; return result; }
    if (std::strcmp(parts[1],"ESC")==0) h.domain=ExtendedTelemetryDomain::ESC;
    else if (std::strcmp(parts[1],"PER")==0) h.domain=ExtendedTelemetryDomain::PERCEPTION;
    else if (std::strcmp(parts[1],"NAV")==0) h.domain=ExtendedTelemetryDomain::NAVIGATION;
    else return result;
    h.group=parts[2];
