...
need('if (next == PeerState::PEER_LOST && !peer_loss_recovery_attempted_)' in peer and 'peer_loss_recovery_attempted_ = true' in peer and 'recoverCan();' in peer and 'if (leaving_peer_lost) peer_loss_recovery_attempted_ = false' in peer,'silent-bus controller recovery is one-shot per peer-loss episode')
