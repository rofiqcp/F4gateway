...
int UsbCdcPort::availableHighPriorityForWrite() const { return RingFree(tx_high_head_, tx_high_tail_, kHighTxSize); }
...
std::size_t UsbCdcPort::writeHighPriority(const uint8_t *data, std::size_t length) {
  if (data == nullptr || length == 0U || length >= kHighTxSize) return 0U;
  const uint32_t primask = __get_PRIMASK();
  __disable_irq();
...
bool UsbCdcPort::writeLineHighPriority(const char *line) {
  ...
  const std::size_t len = std::strlen(line);
  const std::size_t total = len + 2U;
  if (total >= kHighTxSize) return false;
  const uint32_t primask = __get_PRIMASK();
  __disable_irq();
...
bool UsbCdcPort::writeLineCritical(const char *line, uint32_t timeout_ms) {
  ...
  do {
    if (writeLineHighPriority(line)) return true;
    poll();
    HAL_Delay(1U);
...
void UsbCdcPort::poll() {
  ...
  if (use_high && !high_ready) { tx_message_active_ = false; return; }
  if (!use_high && !low_ready) { tx_message_active_ = false; return; }
  ...
  const uint16_t size = use_high ? kHighTxSize : kTxSize;
...
void UsbCdcPort::onTransmitComplete() {
  if (!tx_busy_) return;
  if (tx_active_high_) {
    tx_high_tail_ = static_cast<uint16_t>((tx_high_tail_ + tx_pending_) % kHighTxSize);
  } else {
  ...
  tx_active_high_ = false;
  tx_packet_ends_message_ = false;
  if (message_done) tx_message_active_ = false;
