...
for name,text in [('application',board),('bootloader',boot)]:
    for tok in ('RCC_OSCILLATORTYPE_HSE','RCC_HSE_ON','RCC_PLLSOURCE_HSE','PLLM = 25U','PLLN = 192U','PLLQ = 4U'):
        if tok not in text: raise SystemExit(f'FAIL {name} USB clock: missing {tok}')
    if 'RCC_PLLSOURCE_HSI' in text: raise SystemExit(f'FAIL {name} USB clock regressed to HSI')
