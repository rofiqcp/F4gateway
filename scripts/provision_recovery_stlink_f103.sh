...
probe="$("$OPENOCD" -s "$OPENOCD_SCRIPTS" -f interface/stlink.cfg -f target/stm32f1x.cfg \
  -c 'adapter speed 500; init; halt; echo AGV_DBGMCU_ID=[format 0x%08X [mrw 0xE0042000]]; echo AGV_FLASH_KB=[mrh 0x1FFFF7E0]; resume; shutdown' 2>&1 || true)"
...
"$PYTHON" "$ROOT/scripts/make_app_manifest.py" "$APP" "$MANIFEST" --target f103

[[ $(stat -c %s "$BOOT") -le $((0x4000)) ]] || {
  echo "[F103-STLINK] bootloader exceeds 16 KiB" >&2; exit 5;
}
[[ $(stat -c %s "$APP") -le $((0x3A000)) ]] || {
...
"$OPENOCD" -s "$OPENOCD_SCRIPTS" -f interface/stlink.cfg -f target/stm32f1x.cfg \
...
if [[ -z "$CDC" ]]; then
  ...
  exec_probe="$("$OPENOCD" -s "$OPENOCD_SCRIPTS" -f interface/stlink.cfg -f target/stm32f1x.cfg \
    -c 'adapter speed 500; init; reset run; sleep 1000; halt; shutdown' 2>&1 || true)"
  ...
  echo "[F103-STLINK] application execution verified via SWD PC=$pc_hex"
  exit 0
fi
real="$(readlink -f "$CDC")"
holders="$(fuser "$real" 2>/dev/null || true)"
if [[ -n "$holders" ]]; then
  echo "[F103-STLINK] CDC enumerated; existing holder pid=$holders owns heartbeat verification"
  exit 0
fi
